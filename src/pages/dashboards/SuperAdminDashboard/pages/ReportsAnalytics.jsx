    ...
    try {
      ...
      if (overviewData?.data) {
        setPlatformMetrics(prev => ({
          ...
          subscription: {
            ...prev.subscription,
            totalHospitals: overviewData.data.subscription?.total_hospitals || prev.subscription.totalHospitals,
            activeSubscriptions: overviewData.data.subscription?.active_subscriptions || prev.subscription.activeSubscriptions,
            churnRate: overviewData.data.subscription?.churn_rate || prev.subscription.churnRate,
            newSubscriptions: overviewData.data.subscription?.new_subscriptions || prev.subscription.newSubscriptions,
          },
          financial: {
            ...prev.financial,
            totalRevenue: overviewData.data.financial?.total_revenue || prev.financial.totalRevenue,
            monthlyRecurringRevenue: overviewData.data.financial?.mrr || prev.financial.monthlyRecurringRevenue,
            annualRecurringRevenue: overviewData.data.financial?.arr || prev.financial.annualRecurringRevenue,
          }
        }));
      }
      ...
      if (performanceData?.data) {
        setPlatformMetrics(prev => ({
          ...
          performance: {
            ...prev.performance,
            platformUptime: performanceData.data.metrics?.systemUptime || prev.performance.platformUptime,
            customerSatisfaction: performanceData.data.metrics?.customerSatisfaction || prev.performance.customerSatisfaction,
          }
        }));
      }
      ...
      const subAnalyticsData = await subAnalyticsRes.json().catch(() => ({}));
      ...
      const finAnalyticsData = await finAnalyticsRes.json().catch(() => ({}));

      setAnalyticsData(prev => ({
        ...prev,
        subscriptionGrowth: subAnalyticsData?.data?.growth_chart || prev.subscriptionGrowth,
        planDistribution: subAnalyticsData?.data?.plan_distribution || prev.planDistribution,
        revenueTrends: finAnalyticsData?.data?.revenue_trends || prev.revenueTrends,
      }));
...
const handleTimePeriodChange = (period) => {
  ...
  const fetchPlansList = async () => {
    try {
      const response = await apiFetch(SUPER_ADMIN_SUBSCRIPTION_PLANS);
      const data = await response.json().catch(() => ({}));
      if (response.ok) {
        const raw = data?.data ?? data;
        const items = Array.isArray(raw?.items) ? raw.items : Array.isArray(raw) ? raw : raw?.plans ?? [];
        setPlansList(items);
      }
    } catch (error) {
...
const handleGenerateReport = async (e) => {
  ...
  setTimeout(() => {
    ...
    // alert(successMessage); // Removed alert as per user request
  }, 2000);
                  ...
                  <div>
                    ...
                    >
                      ...
                      {hospitalsList.map((hospital) => (
                        <option key={hospital.id || hospital._id} value={hospital.name}>
                          {hospital.name}
                        </option>
                      ))}
                    </select>
                      ...
                      >
                        ...
                        {hospitalsList.map((hospital) => (
                          <option key={hospital.id || hospital._id} value={hospital.id || hospital._id}>
                            {hospital.name} ({hospital.id || hospital._id})
                          </option>
                        ))}
                      </select>
                    </div>
